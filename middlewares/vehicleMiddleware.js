...
const vehicleMiddleware = async (req, res, next) => {
  ...
  if (!plateNumber || !model || !fuelType) {
    return res.status(400).json({ message: "All fields are required (plateNumber, model, fuelType)." });
  }

  // Validate plate number format (Example: "RAB 123A")
  const plateNumberRegex = /^[A-Z]{3} \d{3}[A-Z]$/;
  if (!plateNumberRegex.test(plateNumber)) {
    return res.status(400).json({ message: "Invalid plate number format. Example: 'RAB 123A'." });
  }

   // Check if vehicle with the same plate number already exists using the repository
   const existingVehicle = await vehicleRepository.getVehicleByPlate(plateNumber);
   if (existingVehicle) {
  ...
  if (!validFuelTypes.includes(fuelType)) {
    return res.status(400).json({ message: `Invalid fuel type. Allowed values: ${validFuelTypes.join(", ")}.` });
  }
